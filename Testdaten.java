...
public class Testdaten {
    ...
    public static void createProdukte(Integer anzahlProdukte) throws SQLException, ClassNotFoundException {
        ...
        for (int i = 0; i <= anzahlProdukte2; i++) {
            double preis = (int) ((Math.random() * 280) + 20) / 100.0;
        ...
        //Werkzeuge
        String[] werkzeuge = {"Hammer", "Motorsäge", "Schlagbohrhammer", "Bewährungsstahl", "Werkbank", "Schraubendreher Kreuzschlitz", "Schraubenzieher Kreuzschlitz", "Polierwerkzeug", "Rasenmäher", "Hochbeet", "Mährobotter", "Hochdruckreiniger", "Kettensäge", "Imbus", "Rohrzange"};
        for (int i = 0; i < anzahlProdukte3; i++) {
            String nummer = "" + random.nextInt(900) + 100;
            double preis = (int) ((Math.random() * 500) + 100) / 100.0;
        ...
        for (int i = 0; i < anzahlProdukte4; i++) {
            double preis = (int) ((Math.random() * 7000) + 1500) / 100.0;
            String name = gender[random.nextInt(gender.length)] + " " +
        ...
        //Fahrräder
        String[] typfahrad = {"Mountenbike", "Rennrad", "Tracking", "Cityrad", "Dreirad", "Hardtail", "E-Bike", "Fixie", "Cyclocross", "Enduro", "All-Mountain", "Downhill", "Time Trial Bike"};
        for (int i = 0; i < anzahlProdukte5; i++) {
            double preis = (int) ((Math.random() * 2501) + 499) / 100.0;
        ...
        for (int i = 0; i < anzahlProdukte6; i++) {
            double preis = (int) ((Math.random() * 295) + 5) / 100.0;
            String name = gender[random.nextInt(gender.length)] + " " +
        ...
        for (int i = 0; i < anzahlProdukte7; i++) {
            double preis = (int) ((Math.random() * 591) + 9) / 10.0;
